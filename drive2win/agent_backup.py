...
U_TURN_TRIGGER     = 2.5   # radians (~143°) — checkpoint is behind, start U-turn
U_TURN_EXIT        = 1.0   # radians (~57°)  — checkpoint is ahead enough, hand back to MLP
...
def make_policy(weights_path: str):
    ...
    def policy(state):
        ...
        # If path ahead is clear, push forward toward checkpoint. Only reverse if wall is close.
        ...
        # U-turn: checkpoint is behind the robot
        if abs(heading_error) > U_TURN_TRIGGER:
            in_uturn[0] = True
        if in_uturn[0]:
            if abs(heading_error) < U_TURN_EXIT:
                in_uturn[0] = False
                print(f"  [agent] U-turn complete — heading_error={heading_error:.2f}")
            else:
                steer = 1.0 if heading_error > 0 else -1.0
                if ray_front > WALL_TRIGGER_DIST:
                    return -0.5, steer   # reverse + steer toward checkpoint
                else:
                    return 0.5, steer    # wall behind, nudge forward instead
